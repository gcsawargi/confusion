  ...
  const renderCommentItem = ({ index, item }) => {
    return (
      <View key={index} style={{ margin: 10 }}>
        <Text style={{ fontSize: 14, fontWeight: 'bold' }}>{item.comment}</Text>
        <Text style={{ fontSize: 13, color: '#888' }}>{item.rating + ' ★'}</Text>
        <Text style={{ fontSize: 13, color: '#444' }}>{'By ' + item.author + ', ' + item.date}</Text>
      </View>
    )
  };
